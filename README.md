...
![Vue3](https://img.shields.io/badge/Vue.js-3.0-brightgreen.svg) ![Spring Boot](https://img.shields.io/badge/SpringBoot-3.2-blue.svg) ![Java](https://img.shields.io/badge/Java-17-orange.svg) ![ECharts](https://img.shields.io/badge/ECharts-6.0-red.svg)

> **毫秒级在线监测 · BFF 数据聚合 · 多级联动告警**
...
- 🕸️ 实时模拟计算 CPU、内存、网络、温湿度等多维指标，前端通过**五维雷达图**呈现设备综合健康画像。
...
```text
senseNode/
├── sense_node-frontend/        # 前端大屏（Vue 3 + Vite + Axios + ECharts）
│   └── src/                    # App.vue、components、assets
├── senseNode-backend/           # 后端服务（Spring Boot + Maven）
│   ├── src/main/java/          # Java 源码（com.example.sensenodebackend）
...
在项目工程化构建过程中，曾遭遇 Spring Boot 3 升级与 Maven `maven-compiler-plugin` 机制变更导致的 Lombok `@Data` 注解大面积失效的底层环境冲突。通过深度排查官方文档与依赖树，最终采用剥离冗余编译插件、交由 Spring Boot 内置接管的方案实现了平滑修复，并总结提炼了完整的排错复盘文档。
...
![senseNode 动态系统架构图](docs/architecture/dynamic-archify-architecture.gif)
